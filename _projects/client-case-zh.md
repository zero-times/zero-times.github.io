...
facts:
  - label: 项目类型
    value: 中小型业务产品
    description: 同时涉及客户端、服务端和交付协同，不是单一模块开发。
  - label: 交付重点
    value: 需求收敛到可上线版本
    description: 把“想做什么”收成“这次必须完成什么”，避免范围失控。
  - label: 公开方式
    value: 匿名化说明
    description: 由于合作边界限制，这里只保留方法和结构，不披露具体业务细节。
highlights:
  - title: 先收敛需求，再谈实现
    description: 这个项目的关键不在于堆功能，而在于先把真正影响上线的部分排出来。
  - title: 客户端和服务端围绕同一目标推进
    description: Flutter 和 Node.js 不是分散推进，而是一起服务于同一个交付版本。
  - title: 把交付准备也算进项目范围
    description: 不是“代码写完”就结束，而是把上线相关配合也纳入实际工作。
deliverables:
  - title: 需求收敛与版本切分
    description: 把原本分散的想法和需求整理成可执行、可上线的版本范围。
  - title: Flutter 客户端推进
    description: 把关键页面、主要流程和可交付体验优先搭起来，保证客户端不是停留在原型层。
  - title: Node.js 补齐与上线配合
    description: 用最小但够用的服务端能力、联调与交付准备，把项目真正推到可上线状态。
decisions:
  - title: 上线优先于功能膨胀
    description: 先做真正影响交付的部分，而不是被不断追加的想法牵着扩大范围。
  - title: 客户端与服务端共享同一个版本节奏
    description: 不是两个团队各自推进，而是围绕同一个交付节点同步决策和实现。
  - title: 对外只保留匿名化方法说明
    description: 案例保留的是结构、判断和工作方式，而不是不适合公开的业务细节。
outcomes:
  - title: 展示了完整交付链路能力
    description: 不只是做一个端，而是把需求、实现和上线准备串起来。
  - title: 说明了我在中小型项目里的价值位置
    description: 更适合在结构尚未完全稳定时，帮助项目快速走到可交付状态。
  - title: 保留了合作边界
    description: 这个案例以匿名化方式展示方法，而不是用不适合公开的细节去换“看起来更完整”。
card_cta_label: "查看案例"
secondary_link_label: "联系我讨论类似合作"
